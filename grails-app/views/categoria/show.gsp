	...
	<body>
		...
		<div id="show-categoria" class="content scaffold-show" role="main">
			...
			<ol class="property-list categoria">
				...
				<g:if test="${categoriaInstance?.empresa}">
				<li class="fieldcontain">
					<span id="empresa-label" class="property-label"><g:message code="categoria.empresa.label" default="Empresa" /></span>
					
						<span class="property-value" aria-labelledby="empresa-label"><g:link controller="tipoEmpresa" action="show" id="${categoriaInstance?.empresa?.id}">${categoriaInstance?.empresa?.encodeAsHTML()}</g:link></span>
